...
import { CategoriaExcecao } from "./categoria.exception";
...
class Categoria extends Entity<ICategoria> implements ICategoria {
    ...
    private set nome(nome: string) {
        ...
        if (nome === null || nome === undefined) {
            throw new CategoriaExcecao.NomeCategoriaNuloOuIndefinido();
        }

        if (tamanhoNome < Categoria.TAMANHO_MINIMO_NOME) {
            throw new CategoriaExcecao.NomeCategoriaTamanhoMinimoInvalido();
        }

        if (tamanhoNome > Categoria.TAMANHO_MAXIMO_NOME) {
            throw new CategoriaExcecao.NomeCategoriaTamanhoMaximoInvalido();
        }
